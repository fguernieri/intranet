...
if (isset($_GET['check_farol']) && isset($_GET['prato_id'])) {
    ...
    $id = intval($_GET['prato_id']);
    $status = 'cinza';

    $query_intranet = "SELECT * FROM ficha_tecnica WHERE id = $id";
    $query_cloud = "SELECT * FROM insumos_bastards WHERE id_ficha = $id";

    $res_intranet = mysqli_query($conn, $query_intranet);
    $res_cloud = mysqli_query($conn_dw, $query_cloud);

    $dados_intranet = mysqli_fetch_assoc($res_intranet);
    $dados_cloud = mysqli_fetch_assoc($res_cloud);

    if ($dados_intranet && $dados_cloud) {
        $status = 'verde';
    } elseif ($dados_intranet || $dados_cloud) {
        $status = 'amarelo';
    } else {
        $status = 'vermelho';
    }
...
<button onclick="rodarFarol()" class="bg-blue-500 text-white px-4 py-2 rounded m-2">Rodar Farol</button>
...
async function rodarFarol() {
    ...
    for (const el of pratos) {
        ...
        const res = await fetch(`consulta.php?check_farol=1&prato_id=${id}`);
